...
static int typeerror(lua_State *L, int narg, const char *expected_name, const char *got_name){
...
T> get_LuaValue(lua_State* L){
    ...
    if(!lua_isuserdata(L, -1)){
        const char* expected_name = get_class_name<type_>(L);
        typeerror(L, -1, expected_name, luaL_typename(L, -1));
    }
    ...
    if(!lua_rawequal(L, -1, -2)){
        const char* expected_name = get_class_name(L, -1);
        lua_pop(L, 1);
        const char* got_name = get_class_name(L, -1);
        lua_pop(L, 1);
        typeerror(L, -1, expected_name, got_name);
    }
    lua_pop(L, 2);
